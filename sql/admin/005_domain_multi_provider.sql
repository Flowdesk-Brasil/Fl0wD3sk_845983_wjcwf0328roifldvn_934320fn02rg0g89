-- Flowdesk domains: Openprovider -> Spaceship, with Cloudflare DNS.
-- Apply after 004_domains.sql.
...
alter table public.domain_dns_records add column if not exists proxied boolean not null default false;
...
create unique index if not exists idx_domains_cloudflare_zone_id
  on public.domains (cloudflare_zone_id) where cloudflare_zone_id is not null;
