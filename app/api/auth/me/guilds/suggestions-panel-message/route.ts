...
const SUGGESTION_START_CUSTOM_ID = "suggestion:start";
...
export async function POST(request: Request) {
  ...
  let diagnostic = createServerSaveDiagnosticContext("suggestions_panel_dispatch");
  ...
  try {
    ...
    const canManage =
      ...
      (dashboardPerms instanceof Set &&
        dashboardPerms.has("server_manage_suggestions_message")) ||
    ...
    if (licenseStatus === "not_paid") {
      const cleanupSummary = await cleanupExpiredUnpaidServerSetups({
        userId: authUserId,
        guildId,
        source: "guild_suggestions_panel_dispatch_post",
      });
    ...
    const payload = buildTicketPanelDispatchPayload(panelLayout, {
      interactiveCustomId: SUGGESTION_START_CUSTOM_ID,
    });
    ...
    if (!listResponse.ok) {
      throw new Error("Falha ao buscar mensagens recentes do canal de sugestoes.");
    }
    ...
    if (!dispatchResponse.ok) {
      ...
      throw new Error(text || "Falha ao enviar o embed de sugestoes.");
    }
    ...
    recordServerSaveDiagnostic({
      ...
      detail: managedMessage
        ? "Embed de sugestoes atualizado com sucesso."
        : "Embed de sugestoes enviado com sucesso.",
    });
