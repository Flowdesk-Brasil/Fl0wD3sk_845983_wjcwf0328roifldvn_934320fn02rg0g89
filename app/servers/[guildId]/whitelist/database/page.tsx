...
export default async function ServersWhitelistDatabasePage({ params }: Props) {
  const workspace = await getServersWorkspaceBootstrap();
  const safeGuildId = normalizeGuildId((await params).guildId);
  if (!safeGuildId) redirect("/servers/");
  return (
    <ServersWorkspace
      ...
      initialTab="settings"
      initialSettingsSection="whitelist_database"
    />
  );
}
