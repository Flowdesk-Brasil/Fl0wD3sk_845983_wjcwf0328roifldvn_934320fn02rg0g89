...
export default function ConfiguracoesPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : "NÃ£o foi possÃ­vel salvar as configuraÃ§Ãµes.");
    }
  }
  async function upload(file?: File) {
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : "NÃ£o foi possÃ­vel enviar o PDF.");
    } finally {
      setUploading(false);
    }
  }
  if (!settings && !error) return <LoadingState label="Carregando configuraÃ§Ãµes..." />;
  return (
    <div className="page-stack">
      <PageHeader eyebrow="Workspace" title="ConfiguraÃ§Ãµes" description="Mantenha os dados operacionais do studio atualizados." />
      <ErrorBanner message={error} />
      {settings && <form onSubmit={submit} className="grid gap-4 xl:grid-cols-[1fr_320px]">
        <section className="card">
          <div className="card-header"><div><h2>Dados do studio</h2><p>InformaÃ§Ãµes utilizadas em documentos e comunicaÃ§Ãµes</p></div><Building2 className="h-5 w-5 text-blue-600" /></div>
          <div className="card-body form-grid">
            ...
            <label><FieldLabel>EndereÃ§o</FieldLabel><input className="field" value={settings.address} onChange={(event) => change("address", event.target.value)} /></label>
          </div>
          <div className="flex justify-end border-t border-[#e3e8f0] p-5"><button className="btn btn-primary"><Save className="h-4 w-4" /> {saved ? "ConfiguraÃ§Ãµes salvas" : "Salvar alteraÃ§Ãµes"}</button></div>
        </section>
        <aside className="grid content-start gap-4">
          <section className="card p-5"><div className="grid h-11 w-11 place-items-center rounded-xl bg-green-50 text-green-600"><ShieldCheck className="h-5 w-5" /></div><h3 className="mt-4 text-sm font-bold">Workspace protegido</h3><p className="mt-2 text-xs leading-5 text-[#657085]">AÃ§Ãµes administrativas sÃ£o registradas na trilha de auditoria.</p></section>
          <section className="card p-5">
            <div className="grid h-11 w-11 place-items-center rounded-xl bg-blue-50 text-blue-600"><FileUp className="h-5 w-5" /></div>
            <h3 className="mt-4 text-sm font-bold">PDF padrÃ£o do contrato</h3>
            <p className="mt-2 text-xs leading-5 text-[#657085]">{settings.contract_template_name ? `Atual: ${settings.contract_template_name}` : "Envie o documento que o aluno verÃ¡ antes de assinar."}</p>
          ...
          {saved && <section className="flex items-center gap-3 rounded-2xl border border-green-200 bg-green-50 p-4 text-xs font-semibold text-green-700"><CheckCircle2 className="h-4 w-4" /> AlteraÃ§Ãµes salvas com sucesso.</section>}
        </aside>
      </form>}
    </div>
  );
