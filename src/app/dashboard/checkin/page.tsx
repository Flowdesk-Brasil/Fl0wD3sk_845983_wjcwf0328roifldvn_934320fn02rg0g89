...
export default function CheckinPage() {
    ...
    const channel = supabase.channel("manual-checkin")
      .on("broadcast", { event: "MANUAL_CHECKIN_APPROVED" }, ({ payload }) => {
        setResult({
          ...
          student_id: "manual",
          status: "allowed",
          reason: "Acesso liberado manualmente pela recepÃ§Ã£o.",
          checked_at: new Date().toISOString(),
          unit: "Matriz",
          student: { full_name: payload.name } as any
        });
        // Atualiza a lista apÃ³s 2 segundos pra dar tempo de registrar
  ...
  async function handleManualRelease(event: FormEvent<HTMLFormElement>) {
    ...
    // Transmite para outros dispositivos (celular admin no modo cÃ¢mera)
  ...
  return (
    <div className="page-stack">
      <PageHeader 
        eyebrow="OperaÃ§Ã£o em tempo real" 
        title="Check-in" 
        description="Valide o acesso por cÃ¢mera, QR Code ou cÃ³digo manual."
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_1fr]">
        <section className="card p-5 sm:p-7">
          <div className="grid min-h-[360px] place-items-center text-center">
            <div className="w-full max-w-md">
              ...
              <p className="mx-auto mt-2 max-w-sm text-xs leading-5 text-[#657085]">Leia o QR Code exibido pelo aluno ou digite o cÃ³digo manualmente.</p>
              <form className="mt-6 flex gap-2" onSubmit={submit}>
                <label className="flex-1"><IconInput icon={Search} autoFocus value={code} onChange={(event) => setCode(event.target.value)} placeholder="CÃ³digo do aluno" /></label>
              ...
              <p className="mt-3 flex items-center justify-center gap-1.5 text-[11px] text-[#8d97aa]"><Clock3 className="h-3.5 w-3.5" /> Leituras repetidas em atÃ© 5 minutos nÃ£o geram outro check-in.</p>
            </div>
          </div>
        </section>
        ...
        <section className="card p-5 sm:p-7">
          <div className={`grid min-h-[360px] place-items-center rounded-2xl border border-dashed text-center ${result?.status === "allowed" ? "border-green-200 bg-green-50/50" : result?.status === "denied" ? "border-red-200 bg-red-50/50" : "border-[#dce3ee] bg-[#fbfcfe]"}`}>
            {!result ? <div><QrCode className="mx-auto h-10 w-10 text-[#c2cad7]" /><h3 className="mt-4 text-sm font-bold">Aguardando validaÃ§Ã£o</h3><p className="mt-1 text-xs text-[#8d97aa]">O resultado da leitura aparecerÃ¡ aqui.</p></div> : (
              <div className="max-w-sm px-5">
                ...
                <h2 className={`mt-5 text-2xl font-extrabold tracking-[-.04em] ${result.status === "allowed" ? "text-green-700" : "text-red-700"}`}>{result.duplicate ? "Check-in jÃ¡ realizado" : result.status === "allowed" ? (result.student_id === "manual" ? "Seja bem-vindo(a)!" : "Acesso liberado") : "Acesso negado"}</h2>
                <p className="mt-2 text-xl font-black text-[#172033]">{result.student?.full_name ?? "CÃ³digo nÃ£o identificado"}</p>
                <p className="mt-1 text-sm text-[#657085]">{result.reason ?? "MatrÃ­cula ativa e acesso regular."}</p>
      ...
      <section className="card">
        <div className="card-header"><div><h2>HistÃ³rico recente</h2><p>Ãšltimas tentativas de acesso</p></div><StatusBadge tone="blue">{history.length} registros</StatusBadge></div>
        <div className="table-wrap"><table className="data-table">
          <thead><tr><th>Aluno</th><th>SituaÃ§Ã£o</th><th className="hide-mobile">Motivo</th><th>Data e hora</th></tr></thead>
          <tbody>{history.map((item) => <tr key={item.id}><td><strong className="text-xs text-[#172033]">{item.student?.full_name ?? "NÃ£o identificado"}</strong></td><td><StatusBadge tone={item.status === "allowed" ? "green" : "red"}>{item.status === "allowed" ? "Liberado" : "Negado"}</StatusBadge></td><td className="hide-mobile">{item.reason ?? "Acesso regular"}</td><td>{formatDateTime(item.checked_at)}</td></tr>)}</tbody>
        </table></div>
      </section>

      <Modal open={manualOpen} onClose={() => setManualOpen(false)} title="LiberaÃ§Ã£o Manual" description="Libere a catraca/acesso registrando o nome do visitante ou aluno.">
        <form onSubmit={handleManualRelease} className="grid gap-4">
          <label>
            ...
            <input 
              ...
              type="text" 
              className="field" 
              placeholder="Ex: JoÃ£o Silva (Visitante)" 
              value={manualName} 
          ...
          <div className="flex justify-end gap-2 mt-2">
            ...
            <button type="submit" className="btn btn-primary" disabled={!manualName.trim()}>Confirmar LiberaÃ§Ã£o</button>
          </div>
        </form>
      </Modal>
    </div>
